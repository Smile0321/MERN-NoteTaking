...
import {
  ...
  HANDLE_ARCHIVE_RECORDS_REQUEST,
  HANDLE_ARCHIVE_RECORDS_FAILURE,
  HANDLE_ARCHIVE_RECORDS_SUCCESS,
} from "./actionTypes";
...
const handleArchiveRequest = () => {
  return { type: HANDLE_ARCHIVE_RECORDS_REQUEST };
};

const handleArchiveSuccess = () => {
  return { type: HANDLE_ARCHIVE_RECORDS_SUCCESS };
};

const handleArchiveFailure = (payload) => {
  return { type: HANDLE_ARCHIVE_RECORDS_FAILURE, payload };
};
...
export const archiveMusicRecords = (id, token, toast) => (dispatch) => {
  ...
  return axios({
    ...
    .then((res) => {
      dispatch(handleArchiveSuccess(id));
      notify(toast, res.data.message, "success");
    })
    .catch((err) => {
      notify(toast, err.response.data.message, "error");
      dispatch(handleArchiveFailure(err));
    });
};